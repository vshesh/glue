...
* all of critic markdown (`{++ins++}`, `{--sub--}`, `{==mark==}` etc)
* `T[text](tooltip)` -> quick n dirty inline tooltip
* `# header` -> a la markdown header
* `![img](url)` -> img like markdown, width=100% though.
...
Glue operates by parsing text using a bag of components called a 
`Registry`. 
